...
const GraphViewModal: React.FunctionComponent<GraphViewModalProps> = ({
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (error.code === 'ECONNABORTED' || error.message?.includes('timeout')) {
        throw new Error('请求超时。数据集可能过大，无法预览。请尝试选择更少的文档。');
      }
      throw new Error(error.response?.data?.error || error.message || '获取图谱数据失败，请重试。');
    }
  ...
  const graphApi = async (mode?: string) => {
    try {
      ...
      if (!result) {
        throw new Error('未收到服务器响应，请求可能已超时。');
      }
      ...
      if (!result.data.data) {
        throw new Error(`响应中没有数据。${result.data.error || '请重试。'}`);
      }
      ...
      if (nodes.length === 0) {
        ...
        setStatusMessage(
          documentCount > 1
            ? `未找到所选 ${documentCount} 个文档的图谱数据。可能是文档仍在处理，或没有可抽取的实体。`
            : `未找到 ${inspectedName} 的图谱数据。该文档可能仍在处理，或没有可抽取的实体。`
        );
        return;
      }
      ...
      setLoading(false);
      setStatus('danger');
      setStatusMessage(error.message || '加载图谱时发生异常。');
    }
  };
        ...
        <Dialog.Content className='flex flex-col n-gap-token-4 w-full grow overflow-auto border! border-palette-neutral-border-weak!'>
          {showLargeDatasetWarning && selectedRows && (
            <Banner
              ...
              description={`你正在预览 ${selectedRows.length} 个文档。大数据集加载时间可能较长并可能超时。为获得更好性能，建议选择更少文档（推荐 4-8 个）。`}
              type='warning'
              usage='inline'
            />
          )}
                ...
                <div className='flex' style={{ height: '100%' }}>
                  <div className='bg-palette-neutral-bg-default relative' style={{ width: '100%', flex: '1' }}>
                    ...
                    <IconButtonArray orientation='vertical' isFloating={true} className='absolute bottom-4 right-4'>
                      ...
                      <IconButtonWithToolTip label='放大' text='放大' onClick={handleZoomIn} placement='left'>
                        <MagnifyingGlassPlusIconOutline className='n-size-token-7' />
                      </IconButtonWithToolTip>
                      <IconButtonWithToolTip label='缩小' text='缩小' onClick={handleZoomOut} placement='left'>
