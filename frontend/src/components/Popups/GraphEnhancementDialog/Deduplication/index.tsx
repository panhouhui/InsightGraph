...
export default function DeduplicationTab() {
  ...
  const selectedFilesCheck = mergeAPIloading
    ...
    : table.getSelectedRowModel().rows.length
      ? `合并重复节点（${table.getSelectedRowModel().rows.length}）`
      : '请选择要合并的节点';
  return (
    <>
      <div>
        <Flex justifyContent='space-between' flexDirection='row'>
          <Flex>
            ...
            <Typography variant={'body-small'}>
              识别并合并类似 “Apple” 和 “Apple Inc.” 的相似实体，减少冗余，提升知识图谱的准确性和清晰度。
            </Typography>
          </Flex>
          {nodesCount > 0 && <Typography variant={'subheading-medium'}>重复节点总数：{nodesCount}</Typography>}
        </Flex>
        <DataGrid
          ref={tableRef}
          isResizable={true}
              ...
              isLoading
                ...
                : !isLoading && !duplicateNodes.length
                  ...
                  : !table.getSelectedRowModel().rows.length
                    ...
                    : mergeAPIloading
                      ? '正在合并'
                      : `合并选中的节点（${table.getSelectedRowModel().rows.length}）`
            }
            label='合并重复节点按钮'
