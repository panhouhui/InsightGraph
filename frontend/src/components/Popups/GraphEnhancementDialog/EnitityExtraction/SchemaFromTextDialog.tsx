...
const SchemaFromTextDialog = ({ open, onClose, onApply }: SchemaFromTextProps) => {
  ...
  const clickHandler = useCallback(async () => {
    ...
    try {
      ...
      if (status === 'Success' && data?.triplets?.length) {
        ...
        showNormalToast('请提供有意义的文本。');
      }
    } catch (error: any) {
      setLoading(false);
      console.error('处理 Schema 出错：', error);
      showNormalToast(error?.message || '发生意外错误。');
    }
  ...
  return (
    <>
      ...
      >
        <Dialog.Header>实体图谱抽取设置</Dialog.Header>
        <Dialog.Content className='n-flex n-flex-col n-gap-token-4'>
          ...
          <div className='flex justify-between mt-4'>
            ...
            <ButtonWithToolTip
              placement='top'
              label='分析按钮'
              text={userText.trim() === '' ? '请填写用于抽取图谱 Schema 的文本' : '分析文本生成 Schema'}
              loading={loading}
