...
import { batchSize, largeFileSize, orderedLlms } from '../utils/Constants';
import { showErrorToast, showNormalToast } from '../utils/Toasts';
import { translateCopy } from '../utils/ChineseCopy';
...
const FileTable: ForwardRefRenderFunction<ChildRef, FileTableProps> = (props, ref) => {
  ...
  const { updateStatusForLargeFiles } = useServerSideEvent(
    (inMinutes, time, fileName) => {
      showNormalToast(`${fileName} 预计需要 ${time} ${inMinutes ? '分钟' : '秒'}`);
    },
    (fileName) => {
      showErrorToast(`${fileName} 处理失败`);
    }
  );
    ...
    () => [
          ...
          if (info.getValue() != 'Processing') {
            return (
              ...
              >
                ...
                <div>{translateCopy(info.getValue())}</div>
              ...
              },
              {
                title: (
                  <span className={`${statusFilter === 'Completed' ? 'n-bg-palette-primary-bg-selected' : ''} p-2`}>
                    <StatusIndicator type='success'></StatusIndicator> 已完成文件
                  </span>
                ),
                onClick: () => {
              ...
              },
              {
                title: (
                  <span className={`${statusFilter === 'New' ? 'n-bg-palette-primary-bg-selected' : 'p-2'} p-2`}>
                    <StatusIndicator type='info'></StatusIndicator> 新文件
                  </span>
                ),
                onClick: () => {
                  setStatusFilter('New');
              ...
              },
              {
                title: (
                  <span className={`${statusFilter === 'Failed' ? 'n-bg-palette-primary-bg-selected' : ''} p-2`}>
                    <StatusIndicator type='danger'></StatusIndicator> 失败文件
                  </span>
                ),
                onClick: () => {
          ...
          if (Number(info.getValue()) === 100 || info.row.original?.status === 'New') {
            return (
              <div className='flex! gap-1 items-center'>
                ...
                <Typography variant='body-medium'>已上传</Typography>
              </div>
            );
            ...
            return (
              <div className='flex! gap-1 items-center'>
                ...
                <Typography variant='body-medium'>无</Typography>
              </div>
            );
          }
          return (
      ...
      columnHelper.accessor((row) => row, {
        ...
        cell: (info) => {
            ...
            return (
              <Flex>
                <span>
                  ...
                  >
                    {translateCopy(info.row.original.fileSource)}
                  </TextLink>
                </span>
              </Flex>
            );
          ...
          return (
            <div>
              <span>{translateCopy(info.row.original.fileSource)}</span>
            </div>
          );
        },
        header: () => <span>来源</span>,
      ...
      columnHelper.accessor((row) => row, {
        ...
        cell: (info) => {
          return (
            <div>
              <span>{translateCopy(info.row.original.type ?? '')}</span>
            </div>
          );
        },
        header: () => <span>类型</span>,
          ...
          columnActions: {
            actions: [
              ...
              ...Array.from(new Set(filesData.map((f) => f.type))).map((t) => {
                return {
                  title: (
                    <span className={`${t === filetypeFilter ? 'n-bg-palette-primary-bg-selected' : ''} p-2`}>
                      {translateCopy(t as string)}
                    </span>
                  ),
                  onClick: () => {
  ...
  useEffect(() => {
    if (connectionStatus && filesData.length && onlyfortheFirstRender && !isReadOnlyUser) {
      ...
      if (processingFilesCount) {
        ...
        showNormalToast('文件正在处理中，请等待上一批完成');
      } else {
        const waitingQueue: CustomFile[] = JSON.parse(
          localStorage.getItem('waitingQueue') ?? JSON.stringify({ queue: [] })
        ).queue;
  ...
  const cancelQueue = async () => {
    ...
    try {
      ...
      if (res.data.status === 'Success') {
        ...
        showNormalToast(`已成功取消 ${queuedFileNames.length} 个等待中的文件`);
      } else {
        throw new Error(res.data.error || '取消队列失败');
      }
    } catch (err) {
      if (err instanceof Error) {
        showErrorToast(`取消队列失败：${err.message}`);
      }
    } finally {
        ...
        prevfiles.map((curfile) => {
          if (curfile.name == fileName) {
            return {
              ...
              errorMessage: errorMessage ?? curfile.errorMessage,
            };
          }
          return curfile;
        })
                  ...
                  return (
                    <DataGridComponents.TableResults>
                      <Flex flexDirection='row' gap='0' alignItems='center'>
                        ...
                        {'当前还没有导入任何数据。请先上传文档或添加网页来源，然后生成知识图谱。'}
                        <span></span>
                      </Flex>
                    </DataGridComponents.TableResults>
                  );
                        ...
                        {queueSize > 0 && (
                          ...
                          >
                            ...
                            <Typography variant='body-medium' className='font-semibold text-palette-warning-text'>
                              {isCancellingQueue
                                ? '正在取消等待队列中的文件...'
                                : `${queueSize} 个文件正在队列中等待`}
                            </Typography>
                            {!isReadOnlyUser && (
                              <IconButtonWithToolTip
                                placement='right'
                                text={isCancellingQueue ? '正在取消...' : '取消所有等待中的文件'}
                                size='small'
                                label='取消队列'
                                clean
