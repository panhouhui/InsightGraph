...
const sanitizeChatMessageText = (value: string) => {
  const text = (value || '').replace(/\r\n/g, '\n').replace(/\r/g, '\n').trim();
  if (!text) {
    return text;
  }

  const chineseStart = text.match(
    /(^|\n)(这个知识图谱|该知识图谱|知识图谱|根据当前|从当前|整体来看|总体来看|以下是|可以从|一、|二、|三、|四、|五、|1[、.])/
  );
  if (chineseStart && chineseStart.index !== undefined && chineseStart.index > 0) {
    const prefix = text.slice(0, chineseStart.index);
    if (/[A-Za-z]{3,}/.test(prefix)) {
      return text.slice(chineseStart.index).trim();
    }
  }

  const lines = text.split('\n');
  const firstChineseLine = lines.findIndex((line) => /[\u4e00-\u9fa5]/.test(line));
  if (firstChineseLine > 0 && lines.slice(0, firstChineseLine).some((line) => /[A-Za-z]{3,}/.test(line))) {
...
const Chatbot: FC<ChatbotProps> = (props) => {
  ...
  const simulateTypingEffect = (messageId: number, response: ResponseMode, mode: string, message: string) => {
    const sanitizedMessage = sanitizeChatMessageText(message);
    const sanitizedResponse = { ...response, message: sanitizedMessage };
    let index = 0;
    ...
    const animate = (timestamp: number) => {
      ...
      if (elapsed >= TYPING_INTERVAL) {
        if (index < sanitizedMessage.length) {
          ...
          const currentTypedText = sanitizedMessage.substring(0, nextIndex);
                ...
                prev.map((msg) =>
                  (msg.id === chatbotMessageId
                    ? {
                        ...
                        modes: {
                          ...msg.modes,
                          [mode]: { ...responseMode, message: sanitizeChatMessageText(responseMode.message) },
                        },
                      }
                    : msg)
                )
            ...
            prev.map((msg) =>
              (msg.id === chatbotMessageId
                ? {
                    ...
                    modes: {
                      ...msg.modes,
                      [mode]: { message: '当前聊天模式获取回答失败。', error: result.reason },
                    },
                  }
                : msg)
            )
                ...
                >
                  ...
                  <Widget
                    ...
                    className={`chat-message-card ${
                      chat.user === 'chatbot' ? 'chat-message-card-bot' : 'chat-message-card-user'
                    } ${isFullScreen ? 'chat-message-card-fullscreen' : ''} ${
                    ...
                    >
                      <div className='chat-message-content prose prose-sm max-w-none'>
                        <ReactMarkdown remarkPlugins={[remarkGfm]} rehypePlugins={[rehypeRaw] as any}>
                          {sanitizeChatMessageText(chat.modes[chat.currentMode]?.message || '')}
                        </ReactMarkdown>
                      </div>
                    </div>
                    <div>
