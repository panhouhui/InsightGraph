...
const Content: React.FC<ContentProps> = ({
  ...
  const extractHandler = async (fileItem: CustomFile, uid: string) => {
    ...
    try {
      const processingModel = model || fileItem.model;
      if (userCredentials && connectionStatus && shouldShowTokenTracking(userCredentials.email)) {
        const tokenCheck = await checkTokenLimits(userCredentials);
        if (tokenCheck.shouldWarn) {
          showNormalToast(tokenCheck.message);
      ...
      if (fileItem.fileSource === 'local file' && fileName.toLowerCase().endsWith('.zip')) {
        const enteredPassword = window.prompt('如果这个 ZIP 压缩包已加密，请输入密码；普通 ZIP 请留空。');
  ...
  const triggerBatchProcessing = (
    ...
    const data = [];
    showNormalToast(`每次处理 ${batch.length} 个文件。`);
  ...
  const disconnect = () => {
    ...
    setAdditionalInstructions(DEFAULT_EXTRACTION_INSTRUCTIONS);
    setMessages([
      {
  ...
  return (
    <>
      ...
      <div className={`n-bg-palette-neutral-bg-default main-content-wrapper`}>
        ...
        <Flex className={`p-2.5  mt-1.5 absolute bottom-0 w-full`} justifyContent='flex-end' flexDirection={'row'}>
