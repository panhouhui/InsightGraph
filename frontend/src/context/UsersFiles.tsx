...
const FileContextProvider: FC<FileContextProviderProps> = ({ children }) => {
  ...
  const availableModels = isProdEnv ? orderedProdLlms : orderedLlms;
  const currentModel =
    selectedModel && availableModels.includes(selectedModel) && selectedModel !== 'gemini_3.5_flash'
      ? selectedModel
      : availableModels.includes(DEFAULT_LLM_MODEL)
        ? DEFAULT_LLM_MODEL
        : availableModels[0];
  ...
  const [selectedTokenChunkSize, setSelectedTokenChunkSize] = useState<number>(
    chunkConfig?.chunkSize && chunkConfig.chunkSize >= tokenchunkSize ? chunkConfig.chunkSize : tokenchunkSize
  );
  const [selectedChunk_overlap, setSelectedChunk_overlap] = useState<number>(
    chunkConfig?.chunkSize && chunkConfig.chunkSize >= tokenchunkSize ? chunkConfig?.chunkOverlap || chunkOverlap : chunkOverlap
  );
  ...
  const [additionalInstructions, setAdditionalInstructions] = useState<string>(
    chunkConfig?.instructions || DEFAULT_EXTRACTION_INSTRUCTIONS
  );
