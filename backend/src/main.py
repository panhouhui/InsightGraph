...
def delete_existing_chunks_for_document(graph, file_name):
  query = """
  MATCH (d:Document {fileName: $filename})
  OPTIONAL MATCH (d)<-[:PART_OF]-(c:Chunk)
  DETACH DELETE c
  WITH d
  SET d.processed_chunk = 0,
      d.total_chunks = 0,
      d.nodeCount = 0,
  ...
  """
  execute_graph_query(graph, query, params={"filename": file_name})
...
async def extract_graph_from_file_local_file(credentials, params, merged_file_path):
  ...
  should_rechunk = params.retry_condition in [START_FROM_BEGINNING, DELETE_ENTITIES_AND_START_FROM_BEGINNING]
  if params.retry_condition in ["", None] or params.retry_condition not in [DELETE_ENTITIES_AND_START_FROM_BEGINNING, START_FROM_LAST_PROCESSED_POSITION] or should_rechunk:
    if should_rechunk:
      graph = create_graph_database_connection(credentials)
      delete_existing_chunks_for_document(graph, params.file_name)
      params.retry_condition = START_FROM_BEGINNING
    if GCS_FILE_CACHE:
  ...
  if len(result) > 0:
    ...
    if len(claimed_result) > 0:
      ...
      if is_uploaded_from_local and not bool(is_cancelled_status):
        ...
        elif get_value_from_env("DELETE_LOCAL_FILE_AFTER_PROCESSING", "False", "bool"):
...
def failed_file_process(uri,file_name, merged_file_path):
  ...
  if GCS_FILE_CACHE:
      ...
      if get_value_from_env("DELETE_FAILED_LOCAL_FILES", False, "bool"):
          logging.info(f'Deleted File Path: {merged_file_path} and Deleted File Name : {file_name}')
          delete_uploaded_local_file(merged_file_path,file_name)
      else:
          logging.info(f'Keeping failed local file for retry: {merged_file_path}')
