...
def save_graphDocuments_in_neo4j(graph: Neo4jGraph, graph_document_list: List[GraphDocument], max_retries=3, delay=1):
   ...
   while retries < max_retries:
       try:
           graph.query("CREATE CONSTRAINT IF NOT EXISTS FOR (b:__Entity__) REQUIRE b.id IS UNIQUE")
           graph.structured_schema = {
               **getattr(graph, "structured_schema", {}),
               "metadata": {
                   **getattr(graph, "structured_schema", {}).get("metadata", {}),
                   "constraint": [{"labelsOrTypes": ["__Entity__"], "properties": ["id"]}],
               },
           }
           _save_graph_documents_without_apoc(graph, graph_document_list)
           return
...
def _escape_cypher_identifier(value: Any, fallback: str) -> str:
...
def _coerce_neo4j_property_value(value: Any):
   ...
   if isinstance(value, (list, tuple, set)):
      ...
      for item in value:
         if item is None or isinstance(item, (str, int, float, bool)):
            ...
            clean_values.append(json.dumps(item, ensure_ascii=False))
      return clean_values
...
def _coerce_neo4j_properties(properties: dict | None) -> dict:
   clean_properties = {}
   for key, value in (properties or {}).items():
      key_text = str(key).strip()
      if not key_text or key_text in {"id"}:
         continue
      clean_properties[key_text] = _coerce_neo4j_property_value(value)
   return clean_properties


def _save_graph_documents_without_apoc(graph: Neo4jGraph, graph_document_list: List[GraphDocument]):
   nodes_by_type = {}
   ...
   for graph_document in graph_document_list:
      for node in graph_document.nodes:
         ...
         node_type = str(getattr(node, "type", "") or "Entity").strip() or "Entity"
         if not node_id:
            continue
         nodes_by_type.setdefault(node_type, []).append({
            "id": node_id,
            "type": node_type,
            "properties": _coerce_neo4j_properties(getattr(node, "properties", {})),
         })

      for relationship in graph_document.relationships:
         source = getattr(relationship, "source", None)
         target = getattr(relationship, "target", None)
         source_id = str(getattr(source, "id", "")).strip() if source is not None else ""
         target_id = str(getattr(target, "id", "")).strip() if target is not None else ""
         rel_type = str(getattr(relationship, "type", "") or "RELATED").strip() or "RELATED"
         ...
         relationships_by_type.setdefault(rel_type, []).append({
            ...
            "target": target_id,
            "type": rel_type,
            "properties": _coerce_neo4j_properties(getattr(relationship, "properties", {})),
         })

   for node_type, rows in nodes_by_type.items():
      node_label = _escape_cypher_identifier(node_type, "Entity")
      graph.query(
         f"""
         UNWIND $rows AS row
   ...
   for rel_type, rows in relationships_by_type.items():
      relationship_type = _escape_cypher_identifier(rel_type, "RELATED")
      graph.query(
         f"""
         UNWIND $rows AS row
         MATCH (source:__Entity__ {{id: row.source}})
         MATCH (target:__Entity__ {{id: row.target}})
