...
1. **Supported LLM Models:**  
   ...
   ```bash
   VITE_LLM_MODELS_PROD="gemini_3.5_flash,openai_gpt_5.4_mini,diffbot,anthropic_claude_4.5_haiku,minimax_m3"
   ```
...
3. **MiniMax International Models:**
   MiniMax uses an OpenAI-compatible endpoint. Add `minimax_m3` to the frontend model list and configure the backend:
   ```bash
   VITE_LLM_MODELS="gemini_3.5_flash,openai_gpt_5.4_mini,diffbot,anthropic_claude_4.5_haiku,minimax_m3"
   LLM_MODEL_CONFIG_MINIMAX_M3="MiniMax-M3,https://api.minimax.io/v1,minimax_api_key"
   ```
...
2. **Run Cloud Build manually:**
   ...
   - `LLM_MODEL_CONFIG_MINIMAX_M3` is the app-facing config key for the `minimax_m3` model option and should map to `MiniMax-M3,https://api.minimax.io/v1,<your-minimax-api-key>`.
